...
class Production(ABC):
    """
    Абстрактный класс расчета добычи для доменной модели
    """
    def __init__(self,
...
class OperationalProductionBalancer(Production):
    """
    Класс балансировщика для оперативного планирования добычи. Для доменной модели, включающей действующий фонд и ВБД
    вычисляется требуемый показатель добычи. Используя оптимизатор итерационно вычисляется вектор дней включения скважин ВБД
    для обеспечения требуемого показателя к нужной дате.

    input:
    case: сценарий балансировки
    ...
    input_paramters: исходные данные по бригадам
    optimizator: класс используемого оптимизатора
    prepared_domain_model: подготовленная доменная модель с суточными показателями добычи нефти и жидкости, FCF
    iterations_count: максимальное количество итераций балансировки

    :returns
    result(): доменная модель с показателями добычи нефти, жидкости и FCF
    optimize(): массив дней включения скважин ВБД
...
class CompensatoryProductionBalancer(OperationalProductionBalancer):
    """
    Класс балансировщика для компенсационных мероприятий добычи.
    Для доменной модели, включающей действующий фонд и ВБД вычисляется начальный показатель накопленной добычи.
    Класс декорирует балансировщик оперативного планирования. Добавляется функционал по отключению нерентабельных скважин
    по информации ГЭП, ограничения по добыче жидкости
    Используя оптимизатор итерационно вычисляется вектор дней включения скважин ВБД
    для обеспечения требуемого показателя накопленной добычи за скользящий/календарный год

    input:
    case=4: сценарий балансировки по накопленной добыче
    input_parameters: исходные данные по бригадам
    optimizator: класс используемого оптимизатора
    prepared_domain_model: подготовленная доменная модель с суточными показателями добычи нефти и жидкости, FCF
    iterations_count: максимальное количество итераций балансировки
