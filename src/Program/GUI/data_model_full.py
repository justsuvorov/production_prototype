...
class DataModelFull(DataModel):
    ...
    def initialization(self):
        ...
        self.min_value['ГПН'] = DataValue(str(0))
        self.max_value['ГПН'] = DataValue(str(100000))
        names = ['Заполярье', 'Шельф', 'Меретояханефтегаз', 'Пальян', 'СПД', 'Арктикгаз', 'Ангара']
        for name in names:
            self.max_value[name] = DataValue(str(500))
        ...
        if self.__five_year_format:
         try:
            self.__five_year_parser = SetOfWellsParserMonth(data_path=self.__path_str, vbd=False)
    ...
    def __load_min_max_for_other_companies(self, forecast_list):
        ...
        for i in range(len(forecast_names)):
            ...
            self.max_value[names[i]].update(value)
    ...
    def save_overal_results(self, path):
        if self.joint_venture:
            ...
            j = 0
        self.__solution.export_overal_results(path=path, solution_index=j, month=self.__constraints.months[self.index], vbd=False)
